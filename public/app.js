        ...
        async function migrateRecipesToDatabase() {
            ...
            for (const key of keys) {
                if (key.startsWith('recipe-') || key.startsWith('special-recipe-')) {
                    const ingredients = localStorage.getItem(key);
                    const type = key.startsWith('special-recipe-') ? 'special' : 'menu';
                    const id = key.startsWith('special-recipe-') ? key.replace('special-recipe-', '') : key.replace('recipe-', '');
                    await fetch(`/api/recipes/${type}/${id}`, {
                        method: 'POST',
        ...
        function showPage(pageId) {
            ...
            if (pageId === 'inventory') { loadInventory(); loadWasteLog(); }
            ...
            if (pageId === 'tools') { loadFiles(); loadLicenses(); loadMaintenanceTasks(); loadRecipesForCalculator(); loadArchives(); loadQRTypeDropdown(); loadEquipmentTracking(); loadReceipts(); }
            if (pageId === 'recipes') loadRecipeBook();
            ...
            if (pageId === 'settings') { loadMeasurementUnits(); loadMenuCategories(); loadInventoryCategories(); loadSupplierCategories(); loadEventStatuses(); loadFileCategories(); loadEmployeeRoles(); loadContactCategories(); loadExpenseCategories(); loadSpecialCategories(); loadQRCategories(); loadEquipmentStatuses(); loadReceiptCategories(); loadDefaultMargin(); loadBusinessInfo(); loadUserProfile(); hideSettingsSections(); }
                    ...
                    form = `
                        <div class="form">
                            <div class="form-group">
                                ...
                                <input type="text" id="recipeName" placeholder="e.g., Birria Tacos">
                            </div>
                            <div class="form-row">
                                <div style="flex: 1;">
                                ...
                                <div style="flex: 1;">
                                    ...
                                    <input type="number" id="portions" placeholder="1" min="1" value="1">
                                </div>
                            </div>
                            <div class="form-row">
                                <div style="flex: 1;">
                    ...
                    form = `
                        <div class="form">
                            <div class="form-group">
                                ...
                                <input type="text" id="ingredientName" placeholder="e.g., Ground Beef">
                            </div>
                            <div class="form-row">
                                <div style="flex: 1;">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="ingredientServings" placeholder="1" min="1">
                            </div>
                            <div class="form-group">
                                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                                    <input type="checkbox" id="isCompound" onchange="toggleCompoundIngredients()" style="width: auto;">
                                    <span>Compound Ingredient (made from other ingredients)</span>
                                </label>
                            </div>
                            <div id="compoundIngredientsSection" style="display: none;">
                                <h4 style="margin: 10px 0;">Sub-Ingredients:</h4>
                                <div id="compoundIngredientsList"></div>
                                <button type="button" class="btn" onclick="addCompoundIngredientRow()" style="background: var(--success); margin: 10px 0;">+ Add Sub-Ingredient</button>
                            </div>
                            <button class="btn" onclick="addIngredient(); closeAddModal();">Save Ingredient</button>
                        </div>
                    `;
                    break;
                case 'inventory':
                    ...
                    form = `
                        <div class="form">
                            <div class="form-group">
                                ...
                                <input type="text" id="inventoryItemName" placeholder="e.g., Paper Towels, Ground Beef">
                            </div>
                            <div class="form-row">
                                <div style="flex: 1;">
                                    <label>Category</label>
                            ...
                            <div class="form-row">
                                <div style="flex: 1;">
                                    ...
                                    <input type="number" id="currentStock" placeholder="0" step="0.1">
                                </div>
                                <div style="flex: 1;">
                                    <label>Min Stock</label>
                                    <input type="number" id="minStock" placeholder="0" step="0.1">
                                </div>
                            </div>
            ...
            modalForm.innerHTML = `
                <div class="form">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 15px;">
                        <input type="text" id="editEmployeeFoodHandlerCard" placeholder="Food Handler Card #" value="${employee.food_handler_card_number || ''}" style="width: 100%;">
                        <input type="date" id="editEmployeeFoodHandlerExpiration" placeholder="Card Expiration" value="${employee.food_handler_expiration || ''}" style="width: 100%;">
                    </div>
                    <div style="margin-bottom: 8px;">
                        <label style="display: block; margin-bottom: 5px; font-size: 0.9em;">Food Handler Card Photo:</label>
                        ${employee.food_handler_photo ? `<img src="${employee.food_handler_photo}" style="max-width: 200px; border-radius: 4px; margin-bottom: 8px;">` : ''}
                        <input type="file" id="editEmployeeFoodHandlerPhoto" accept="image/*" style="width: 100%;">
                    </div>
                    <div class="form-group">
                        <textarea id="editEmployeeNotes" placeholder="Notes" rows="2">${employee.notes || ''}</textarea>
                    </div>
        ...
        function updateEmployee(id) {
            ...
            const phone = document.getElementById('editEmployeePhone').value.trim();
            const address = document.getElementById('editEmployeeAddress').value.trim();
            const food_handler_card_number = document.getElementById('editEmployeeFoodHandlerCard').value.trim();
            const food_handler_expiration = document.getElementById('editEmployeeFoodHandlerExpiration').value;
            const food_handler_photo_file = document.getElementById('editEmployeeFoodHandlerPhoto').files[0];
            const notes = document.getElementById('editEmployeeNotes').value.trim();
            
            const updateEmployee = (photoPath = null) => {
                if (employeeIndex !== -1) {
                    ...
                    employees[employeeIndex].food_handler_card_number = food_handler_card_number;
                    employees[employeeIndex].food_handler_expiration = food_handler_expiration;
                    if (photoPath) employees[employeeIndex].food_handler_photo = photoPath;
